    ...
    <header className="sticky top-0 z-50 border-b border-border bg-background/85 backdrop-blur-md">
      <div className="mx-auto grid max-w-[1180px] grid-cols-[minmax(0,1fr)_auto] items-center gap-3 px-4 py-3 sm:px-6">
        ...
        <div className="flex shrink-0 items-center gap-2">
          <nav className="mr-2 hidden items-center gap-6 text-sm font-semibold md:flex">
          ...
          <a
            className="btn-base bg-whatsapp px-4 py-2.5 text-xs text-whatsapp-foreground"
            href={waLink(config)}
            target="_blank"
            rel="noreferrer"
          >
...
export function SiteFooter({ config }: { config: SiteConfig }) {
  return (
    <footer className="border-t border-border px-4 py-10 text-center text-[13px] text-muted-foreground">
