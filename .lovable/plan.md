...
Ese mensaje aparece cuando falla la creación del pedido en la base, antes de tocar Mercado Pago (o sea: el problema no es el procesador de pagos, es la escritura del pedido).

Dos causas posibles y hay que confirmarlas antes de tocar código:

- En Vercel están cargadas las variables de Mercado Pago, pero el registro del pedido usa otras dos: la URL del backend y la clave de servicio (service role). Si falta alguna, el pedido no se guarda y sale exactamente ese mensaje.
- El proyecto de backend nuevo puede no tener la tabla de pedidos con los permisos correctos.

Pasos:

1. Hacer que el error deje de ser genérico: mostrar/loguear el motivo real (falta de configuración vs. error de la base vs. permisos), así se identifica en un intento.
2. Revisar en Vercel que estén cargadas las variables del backend nuevo (URL, clave pública y clave de servicio) además de las de Mercado Pago. Te digo exactamente cuáles y dónde ponerlas.
...
Nota importante: el entorno de Lovable sigue apuntando al backend anterior, mientras Vercel apunta al nuevo. Conviene decidir cuál queda como definitivo para no seguir con dos bases distintas.

## 2. Los datos del usuario no llegan a la base

Al registrarse, los datos (nombre, DNI, teléfono, provincia, ciudad, código postal, sucursal) se envían junto al alta de la cuenta y un automatismo del backend los copia a la tabla de perfiles. Ese automatismo existe en el backend viejo; en el nuevo hay que recrearlo. El email y la contraseña viven en el sistema de cuentas del backend (la contraseña siempre encriptada, no se puede guardar en texto).

Pasos:

1. Recrear en el backend nuevo el automatismo que crea el perfil al registrarse, más los permisos de la tabla.
2. Además, al iniciar sesión y al confirmar un pedido, guardar/actualizar el perfil desde la app: así, incluso si el automatismo no corrió, el perfil queda completo.
